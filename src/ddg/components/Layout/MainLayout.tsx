...
export const MainLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="app-shell flex flex-column h-screen overflow-hidden">
      <Header onToggleSidebar={handleToggleSidebar} isMobile={isMobile} />

      <div className="app-shell__body flex flex-1 overflow-hidden">
        {!isMobile && (
          <aside
            className={`app-shell__sidebar h-full transition-all transition-duration-300 overflow-hidden ${collapsed ? "w-4rem" : "w-18rem"}`}
          >
        ...
        <div className="app-shell__content flex flex-column flex-1 overflow-hidden">
          <main className="app-shell__main flex-1 overflow-y-auto p-3 md:p-4">
            {children}
          </main>
          {!isMobile && <Footer />}
        </div>
