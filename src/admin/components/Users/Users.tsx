...
export const Users = () => {
  ...
  const { users, handleCreate, handleUpdate, handleDelete } =
    useUsersDataGrid();
  ...
  const isManagerBody = (rowData: any) => {
    ...
    return (
      <Tag
        value={rowData.is_manager ? "Lider" : "Servidor"}
        icon={icon}
        style={style}
        rounded
      />
    ...
    return (
      <>
        ...
        <Button
          ...
          onClick={() => handleOpenChangePassword(rowData)}
          style={{ marginLeft: "0.5rem", height: "2rem", width: "2rem" }}
        />
        <Button
          icon="pi pi-trash"
          rounded
  ...
  const userCard = (rowData: IUser) => (
    <div key={rowData.user_id} className="admin-mobile-card p-3 mb-3">
      ...
      <div className="admin-mobile-card__actions flex align-items-center justify-content-end gap-2 mt-3">
        <Button
          icon="pi pi-pencil"
          rounded
          severity="success"
          className="admin-mobile-card__action-btn"
          aria-label="Editar"
          onClick={() => editUser(rowData)}
          style={{ height: "2rem", width: "2rem" }}
        />
  ...
  return (
    <MainLayout>
        ...
        <div className="admin-mobile-card-list">
          <div className="flex flex-column gap-2 mb-3">
            ...
            <Button
              ...
              label="Agregar Usuario"
              onClick={() => openNew()}
              className="w-full admin-mobile-card__add-btn"
            />
          </div>
      ...
      >
        <div className="field">
          <label htmlFor="user_nm" className="font-bold">
            Nombre
          </label>
          <InputText
            id="user_nm"
            value={user.user_nm}
            onChange={(e) => onInputChange(e, "user_nm")}
            required
            autoFocus
            className={classNames({ "p-invalid": submitted && !user.user_nm })}
          />
          {submitted && !user.user_nm && (
            <small className="p-error">El nombre es obligatorio.</small>
          )}
        </div>
        <div className="field">
          <label htmlFor="user_lt" className="font-bold">
            Apellido
          </label>
          <InputText
            id="user_lt"
            value={user.user_lt}
            onChange={(e) => onInputChange(e, "user_lt")}
            required
          />
        </div>
        <div className="field">
        ...
        <div className="field flex gap-2 align-items-center">
          <Checkbox
            ...
            onChange={(e) => setUser({ ...user, is_active: e.checked })}
          />
          ...
          <Checkbox
            ...
            onChange={(e) => setUser({ ...user, is_manager: e.checked })}
          />
          <label htmlFor="is_manager" className="font-bold mb-0">
            Lider
          </label>
        </div>
      </Dialog>
