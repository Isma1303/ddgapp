...
import type {
  IRoleAssignament,
  IUserDepartmentAssignament,
} from "../../interfaces/assignament.interface";
...
export const Assignaments = () => {
  ...
  const [managerDepartmentId, setManagerDepartmentId] = useState<number | null>(
    null,
  );
  ...
  >([]);
  const [selectedReportsToUserId, setSelectedReportsToUserId] = useState<
    number | null
  >(null);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [usersData, rolesData, assignmentsData, departmentsData] =
        await Promise.all([
          authService.getAll(),
          rolesService.getAll(),
      ...
      const departmentData = await assignamentsService
        .getAssign()
        .catch(() => []);
      ...
      setDepartmentAssignments(
        toArray<IUserDepartmentAssignament>(departmentData),
      );
    ...
    try {
      const exists = roleAssignments.some(
        (a) =>
          ...
          a.role_id === selectedRole.role_id,
      );
      if (exists) {
        toast.current?.show({
          severity: "info",
  ...
  const isManagerAssignment = (assignment: IUserDepartmentAssignament) => {
    ...
    const assignmentAsDepartmentHead =
      assignment.reports_to === 0 ||
      assignment.reports_to === assignment.user_id;
    const user = users.find((item) => item.user_id === assignment.user_id);
    const assignmentByUserFlag = user?.is_manager === true;

    return (
      assignmentAsLeader || assignmentAsDepartmentHead || assignmentByUserFlag
    );
  };
  ...
  const managerAssignments = useMemo(
    () =>
      assignmentsWithUserData.filter((assignment) =>
        isManagerAssignment(assignment),
      ),
  ...
  const memberAssignments = useMemo(
    () =>
      assignmentsWithUserData.filter(
        (assignment) => !isManagerAssignment(assignment),
      ),
    [assignmentsWithUserData],
  );

  const managersByDepartment = useMemo(() => {
    return managerAssignments.reduce<
      Record<number, IUserDepartmentAssignament[]>
    >((acc, assignment) => {
  ...
  const departmentOptions = useMemo(() => {
    return departments
      ...
      .sort((first, second) =>
        first.department_nm.localeCompare(second.department_nm),
      )
      .map((department) => ({
  ...
  const handleAssignManager = async () => {
    if (!managerDepartmentId || !selectedManagerUserId) {
      toast.current?.show({
        severity: "warn",
        summary: "Atención",
        detail: "Seleccione departamento y manager",
      });
      return;
    }
    ...
    const duplicate = departmentAssignments.some(
      (assignment) =>
        assignment.department_id === managerDepartmentId &&
        assignment.user_id === selectedManagerUserId,
    );
    ...
    if (duplicate) {
      toast.current?.show({
        severity: "info",
        summary: "Info",
        detail: "El usuario ya está asignado en este departamento",
      });
      return;
    }
    ...
    try {
      ...
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: "Manager asignado correctamente",
      });
      resetDepartmentForm();
      void loadData();
    } catch (error) {
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: "No se pudo asignar el manager",
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const handleAssignMember = async () => {
    if (
      ...
      !selectedReportsToUserId
    ) {
      toast.current?.show({
        severity: "warn",
        summary: "Atención",
        detail: "Complete departamento, integrante y manager",
      });
      return;
    }
    ...
    const duplicate = departmentAssignments.some(
      (assignment) =>
        assignment.department_id === memberDepartmentId &&
        assignment.user_id === selectedMemberUserId,
    );
    ...
    try {
      ...
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: "Integrante asignado correctamente",
      });
      resetDepartmentForm();
      void loadData();
    } catch (error) {
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: "No se pudo asignar el integrante",
      });
    } finally {
      setActionLoading(false);
    }
  ...
  const itemTemplate = (user: User) => {
    ...
    return (
      <div className="col-12 md:col-6 lg:col-4 p-2">
        <div className="assignaments-user-card p-4 h-full flex flex-column">
          <div className="flex align-items-center justify-content-between mb-3">
            ...
            <Button
              ...
              text
              severity="secondary"
              className="assignaments-icon-btn"
          ...
          <div className="flex-grow-1">
            <div className="block assignaments-label mb-2 text-sm">
              Roles asignados
            </div>
              ...
              {userRoles.length > 0 ? (
                userRoles.map((role: any) => (
                  <Tag
                    ...
                    className="py-1 px-2 assignaments-role-tag"
                    severity="info"
                    rounded
                  >
                ...
                <span className="text-500 text-sm font-italic">
                  Sin roles asignados
                </span>
              )}
            </div>
  ...
  const groupedMemberAssignments = useMemo(() => {
    return memberAssignments.reduce<
      Record<number, IUserDepartmentAssignament[]>
    >((acc, assignment) => {
      const departmentId = assignment.department_id;
      if (!acc[departmentId]) {
  ...
  return (
    <MainLayout>
      <Toast ref={toast} />
      <TabView className="assignaments-tabview">
        <TabPanel header="Asignar Roles" leftIcon="pi pi-person-add">
          <div className="assignaments-page-header mb-3 md:mb-4">
            <h1 className="m-0 app-page-title">Gestión de Asignaciones</h1>
          ...
          {loading ? (
            ...
            <DataView
              ...
              paginator
              rows={isMobile ? 6 : 9}
              className="assignaments-dataview"
            />
          )}
        ...
        <TabPanel header="Asignar Ministerios" leftIcon="pi pi-sitemap">
          <div className="assignaments-page-header mb-4">
            <h1 className="m-0 app-page-title">Asignaciones por ministerio</h1>
            <p className="app-page-subtitle">
              Administra usuarios como lideres e integrantes por ministerios.
            </p>
          </div>

          <TabView className="assignaments-subtabview">
            <TabPanel header="Lideres" leftIcon="pi pi-user-plus">
              <div className="assignaments-form-card p-4 mb-4">
                <div className="grid formgrid">
                  <div className="field col-12 md:col-4">
                    ...
                    <Dropdown
                      ...
                      onChange={(event) =>
                        setManagerDepartmentId((event.value as number) ?? null)
                      }
                  ...
                  <div className="field col-12 md:col-5">
                    ...
                    <Dropdown
                      ...
                      onChange={(event) =>
                        setSelectedManagerUserId(event.value as number)
                      }
              ...
              {groupedManagerAssignments.length === 0 ? (
                <div className="assignaments-empty text-500 font-italic">
                  Sin lideres asignados todavía.
                </div>
              ) : (
                groupedManagerAssignments.map(([departmentId, assignments]) => (
                  <div
                    key={departmentId}
                    className="assignaments-group-card p-3 mb-3"
                  >
                    <div className="font-bold mb-2">
                      {formatDepartment(Number(departmentId))}
                    </div>
                    <div className="flex flex-wrap gap-2">
            ...
            <TabPanel header="Integrantes" leftIcon="pi pi-users">
                ...
                <div className="grid formgrid">
                  ...
                  <div className="field col-12 md:col-4">
                    ...
                    <Dropdown
                      ...
                      onChange={(event) =>
                        setSelectedMemberUserId(event.value as number)
                      }
              ...
              {Object.keys(groupedMemberAssignments).length === 0 ? (
                ...
                Object.entries(groupedMemberAssignments).map(
                      ...
                      <div className="flex flex-column gap-2">
                        {assignments.map((assignment) => {
                          const manager = users.find(
                            (user) => user.user_id === assignment.reports_to,
                          );
                          return (
                            <div
                              key={`${assignment.department_id}-${assignment.user_id}`}
                              className="assignaments-member-row flex flex-column md:flex-row md:align-items-center justify-content-between gap-2 p-2"
                            >
                              <span>
                                {assignment.user_nm ??
                                  `Usuario ${assignment.user_id}`}
                              ...
                              <Tag
                                value={`Manager: ${manager?.user_nm ?? `Usuario ${assignment.reports_to}`}`}
                                severity="info"
                              />
                            </div>
                          );
